...
firebase.auth().onAuthStateChanged(async function(user) {
  let apiKey = '492b34539ebbf57b914c2d147b69db01'
  ...
  if (user) {
    ...
    document.querySelector('.sign-in-or-sign-out').innerHTML = `
      ...
      <button class="block-center text-center text-white bg-green-500 mt-4 px-4 py-2 rounded sign-out">Sign Out</button>
    `
    ...
    for (let i=0; i<movies.length; i++) {
      let movie = movies[i]
      let docRef = await db.collection('watched_movies').doc(`${movie.id}`).get()
